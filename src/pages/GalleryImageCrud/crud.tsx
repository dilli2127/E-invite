...
const formColumns = 2;
const { Option } = Select;
const GalaryImage: React.FC = () => {
  const getRoute = getApiRouteCmsImage("GetAllGalleryImages");
  ...
  const handleApiResponse = (
    ...
    if (success) {
      showToast("success", `Gallery Image ${action}d successfully`);
      getAllGallery();
      resetForm();
      ...
      showToast("error", `Failed to ${action} gallery image`);
    }
  };
...
export default memo(GalaryImage);
