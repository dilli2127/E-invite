import React, { useCallback, useEffect } from "react";
import { Card, Row, Col } from "antd";
import { useNavigate } from "react-router-dom";
import { getApiRouteEAlbum } from "../../helpers/Common_functions";
import { ApiRequest } from "../../services/api/apiService";
import { dynamic_request, useDynamicSelector } from "../../services/redux";
import { useDispatch } from "react-redux";
import { Dispatch } from "redux";
import { EAlbum } from "../../routes/types/routeConfig";

const EAlbumLanding: React.FC = () => {
  const navigate = useNavigate();
  const dispatch: Dispatch<any> = useDispatch();

  const callBackServer = useCallback(
    (variables: ApiRequest, key: string) => {
      dispatch(dynamic_request(variables, key));
    },
    [dispatch]
  );

  const getRoute = getApiRouteEAlbum("GetAll");
  const { loading, items } = useDynamicSelector(getRoute.identifier);
  const galleryItems: EAlbum[] = items?.result || [];

  const getAllEAlbum = useCallback(() => {
    callBackServer(
      { method: getRoute.method, endpoint: getRoute.endpoint, data: {} },
      getRoute.identifier
    );
  }, [callBackServer, getRoute]);

  useEffect(() => {
    getAllEAlbum();
  }, [getAllEAlbum]);

  const openGallery = (albumId: number) => {
    const selectedAlbum = galleryItems.find((album) => album._id === albumId);
    if (selectedAlbum) {
      navigate(`/ealbum/${albumId}`, {
        state: { pdfUrl: selectedAlbum.albumurl },
      });
    }
  };

  if (loading) {
    return <div style={centerStyle}>Loading...</div>;
  }

  if (!galleryItems.length) {
    return (
      <div style={centerStyle}>
        No Albums Found
        <br />
        Please Contact Fresh Focuz Studio
        <br />
        9677894094
        <br />
        freshfocuzstudio@gmail.com
      </div>
    );
  }

  return (
    <div style={{ padding: 20 }}>
      <h1 style={{ textAlign: "center", marginBottom: 20 }}>E Album</h1>
      <Row gutter={[16, 16]}>
        {galleryItems.map((album) => (
          <Col xs={24} sm={12} md={8} lg={6} key={album._id}>
            <Card
              hoverable
              cover={
                <img
                  alt={album.albumname}
                  src={album.albumcover}
                  style={imageStyle}
                />
              }
              onClick={() => openGallery(album._id)}
            >
              <Card.Meta title={album.albumname} />
            </Card>
          </Col>
        ))}
      </Row>
    </div>
  );
};

const centerStyle: React.CSSProperties = { textAlign: "center", marginTop: 20 };

const imageStyle: React.CSSProperties = {
  height: "200px",
  objectFit: "cover",
  borderRadius: "8px",
  minHeight: "200px",
};

export default EAlbumLanding;
