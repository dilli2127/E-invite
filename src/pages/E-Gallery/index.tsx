import React, { useCallback, useEffect } from "react";
import { Card, Row, Col } from "antd";
import { useNavigate } from "react-router-dom";
import {
  getApiRouteEGallery,
  getApiRouteGallarey,
} from "../../helpers/Common_functions";
import { ApiRequest } from "../../services/api/apiService";
import { dynamic_request, useDynamicSelector } from "../../services/redux";
import { useDispatch } from "react-redux";
import { Dispatch } from "redux";
import { EAlbum } from "../../routes/types/routeConfig";

const AlbumPage: React.FC = () => {
  const navigate = useNavigate();
  const dispatch: Dispatch<any> = useDispatch();
  const callBackServer = useCallback(
    (variables: ApiRequest, key: string) => {
      dispatch(dynamic_request(variables, key));
    },
    [dispatch]
  );
  const getRoute = getApiRouteEGallery("GetAll");
  const { loading, items } = useDynamicSelector(getRoute.identifier);

  const galleryItems: EAlbum[] = items?.result || [];
  const getAllGallery = () => {
    callBackServer(
      { method: getRoute.method, endpoint: getRoute.endpoint, data: {} },
      getRoute.identifier
    );
  };
  const openGallery = (albumId: number) => {
    const album = galleryItems.find((item) => item._id === albumId);
    if (album) {
      navigate(`/egallery/${albumId}`, {
        state: { FOLDER_ID: album?.drive_folder_id },
      });
    }
  };
  useEffect(() => {
    getAllGallery();
  }, []);
  return (
    <>
      {items?.result?.length > 0 ? (
        <div style={{ padding: 20 }}>
          <h1 style={{ textAlign: "center", marginBottom: 20 }}>E Gallery</h1>
          <Row gutter={[16, 16]}>
            {galleryItems.map((album) => (
              <Col xs={24} sm={12} md={8} lg={6} key={album._id}>
                <Card
                  hoverable
                  cover={
                    <img
                      alt={album.name}
                      src={album.album_cover}
                      style={{
                        height: "200px",
                        objectFit: "cover",
                        borderRadius: "8px",
                        minHeight: "200px",
                      }}
                    />
                  }
                  onClick={() => openGallery(album._id)}
                >
                  <Card.Meta title={album.name} />
                </Card>
              </Col>
            ))}
          </Row>
        </div>
      ) : (
        <div style={{ textAlign: "center", marginTop: 20 }}>
          No Gallery Found
          <br />
          Please Contact Fresh Focuz Studio
          <br />
          9677894094
          <br /> freshfocuzstudio@gmail.com
        </div>
      )}
      {loading && (
        <div style={{ textAlign: "center", marginTop: 20 }}>Loading...</div>
      )}
    </>
  );
};

export default AlbumPage;
